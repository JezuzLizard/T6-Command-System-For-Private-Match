...
private in_function_main( aitype_parse_obj )
{
	for ( ;; )
	{
		...
		if ( !_istricmp( trimmed_line, "switch" ) )
		{
			in_switch = 1;
		...
		else if ( !_istricmp( trimmed_line, "self " ) )
		{
			start = 5;
		...
		else if ( !_istricmp( trimmed_line, "randchar" ) )
		{
			continue;
		}
...
parse_radiant_keys()
{
	...
	for ( ;; )
	{
		...
		_RESET_FINAL_STR();
		if ( _ASSERT_MSG( remaining > 0, "1Early end to radiant line parse Line: '{}', ParseToken: '{}', Remaining '{}'", level._fmt_str, type, remaining ) )
		{
			continue;
		}
		...
		if ( _ASSERT_MSG( isdefined( level._radiant_key_types_keys[ type ] ), "Invalid type '{}', Line: '{}', ParseToken: '{}', Remaining '{}'", type, level._fmt_str, type, remaining ) )
		{
			continue;
		}
		...
		_RESET_FINAL_STR();
		if ( _ASSERT_MSG( remaining > 0, "2Early end to radiant line parse Line: '{}', ParseToken: '{}', Remaining '{}'", level._fmt_str, level._fmt_final_str, remaining ) )
		{
			continue;
		}
		...
		// remaining = _PUSH_POS_UNTIL_PREDICATE( ::is_alpha_numeric, false, true );
		// if ( _ASSERT_MSG( remaining > 0, "5Early end to radiant line parse Line: '{}', ParseToken: '{}', Remaining '{}'", level._fmt_str, level._fmt_final_str, remaining ) )
		// {
		// 	continue;
		// }
