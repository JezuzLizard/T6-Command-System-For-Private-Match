...
private cmd_spawn_f( param )
{
	...
	{
		case "trigger_radius":
			ent = self spawn_trigger_radius( origin, spawnflags, param.a[ 3 ], param.a[ 4 ] );
			break;
		case "trigger_box":
			ent = self spawn_trigger_box( origin, spawnflags, param.a[ 3 ], param.a[ 4 ], param.a[ 5 ] );
			break;
		case "trigger_box_use":
			ent = self spawn_trigger_box_use( origin, spawnflags, param.a[ 3 ], param.a[ 4 ], param.a[ 5 ] );
			break;
		case "trigger_radius_use":
			ent = self spawn_trigger_radius_use( origin, spawnflags, param.a[ 3 ], param.a[ 4 ] );
			break;
		case "trigger_damage":
			ent = self spawn_trigger_damage( origin, spawnflags, param.a[ 3 ], param.a[ 4 ] );
			break;
		case "script_model":
			ent = self spawn_script_model( origin, spawnflags );
			break;
		case "script_origin":
...
fs_write_kvps( fh, mapent )
{
	...
	for ( i = 0; i < _SIZE( keys.size ); i++ )
	{
		switch ( keys[ i ] )
		{
			case "origin":
			case "angles":
				fs_writeline( fh, "\"" + keys[ i ] + "\" \"" + get_mapents_vector( mapent.keys[ keys[ i ] ] ) + "\"" );
				continue;
		}

		fs_writeline( fh, "\"" + keys[ i ] + "\" \"" + mapent.keys[ keys[ i ] ] + "\"" );
	}
}

fs_write_mapent_entry( fh, mapent )
{
	fs_writeline( fh, "{" );
	fs_write_kvps( fh, mapent );
	fs_writeline( fh, "}" );
...
private cmd_savemapents_f( param )
{
	...
	namespaces = getarraykeys( level._mapents );
	for ( i = 0; i < _SIZE( namespaces.size ); i++ )
	{
		keys = getarraykeys( level._mapents[ namespaces[ i ] ] );
		for ( j = 0; j < _SIZE( keys.size ); j++ )
		{
			mapent = level._mapents[ namespaces[ i ] ][ keys[ j ] ];
			fs_write_mapent_entry( paths_file, mapent );
		}
	}
...
private cmd_editmapentfield_f( param )
{
	mapent_type = param.a[ 0 ];
	id = param.a[ 1 ];
	fieldname = param.a[ 2 ];
	fieldvalue = param.a[ 3 ];
	scale = _DEFAULT( param.a[ 4 ], 1.0 );
	is_relative = _DEFAULT( param.a[ 5 ], false );
	...
	if ( assign_result.errored )
	{
		return param add_executor_cmderror( assign_result.msg );
	}

	return param add_executor_cmdinfo( assign_result.msg );
}