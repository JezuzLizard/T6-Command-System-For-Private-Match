...
CMD_UTILITY_VOTELIST_f( arg_list )
{
	...
	for ( i = 0; i < voteables_keys.size; i++ )
	{
		...
		remaining_cmds--;
		if ( ( voteables_to_display.size > level.commands_page_max ) && remaining_cmds != 0 )
		{
			if ( current_page == user_defined_page )
			{
				...
				level COM_PRINTF( channel, "cmdinfo", "Displaying page " + current_page + " out of " + level.commands_page_count + " do /showmore or /page <num> to display more voteables.", self );
				...
				else if ( isSubStr( result[ 0 ], "page" ) )
				{
					...
					if ( user_defined_page > level.commands_page_count || user_defined_page == 0 )
					{
						level COM_PRINTF( channel, "cmderror", "Page number " + result[ 1 ] + " sent to votelist is invalid. Valid inputs are 1 thru " + level.commands_page_count, self );
						return;
					}
				}
				else if ( result[ 0 ] == "showmore" )
